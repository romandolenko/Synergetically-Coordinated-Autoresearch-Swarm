...
> Consolidated write-up. Day-by-day detail in [`results-day3.md`](results-day3.md) and [`results-day4.md`](results-day4.md). **Post-review hardening (fair controls, performance-aware variant, validated embedding mitigation) in [`results-review.md`](results-review.md) — read it before acting on the headline ACs.** **The *genuine* SCT dynamical law (integrated `T·ψ̇+ψ=u` + relaxing order parameter), a SCT+novelty-search hybrid, and the engineered heuristic are now compared head-to-head with bootstrap CIs in [`results-laws.md`](results-laws.md): the SCT law contributes *emergent* cohesion (AC-B3 lift +1.11, no scripted anneal) but coverage is a *search* property — adding novelty search (hybrid) lifts coverage marginal 0.78→1.23× yet still trails the engineered gate (1.58× slower). A `psi_0` sweep shows the unscripted law faces an *irreducible single-knob coverage↔cohesion Pareto tradeoff* (no setting clears marginal ≥1.5 and lift ≥0.5 jointly); the heuristic escapes it only by *time-separating* the two objectives via its scripted anneal. With CIs, no arm robustly clears the 1.5× coverage bar. A `w_p` sweep (turning the objective on) shows performance-awareness *neither* improves best_val (flat within noise) *nor* escapes the tradeoff (it's a redundant cohesion knob; `w_p=50` re-triggers the saturation trap) — the coordinator reshapes *where* the swarm searches, not *how good* the best solution is.** Spec docs: [`requirements.md`](requirements.md), [`design.md`](design.md), [`tasklist.md`](tasklist.md).
...
| **AC-B2** (coverage, control-law marginal ≥ 1.5) | ratio ≥ 1.5 vs **fair partition control** (official) | committed defaults **1.39× FAIL**; **coordination variant PASS** (1.94× at K'=5/G=3; 1.6–1.9× at K'=10/G≥4) | **committed FAIL; PASS via the flagged coordination variant** (multi-agent + embedding-repulsion + gating) when **G ≳ K'/3** — see [`results-review.md`](results-review.md) §6c, §6e |
...
| **AC-B4** (knob monotonicity) | \|τ\| ≥ 0.7 | **τ = +1.000 aggregate**, +0.770 per-seed flat | **PASS** (mechanical; 5 T-points) |
